...
def solve_bfs_from_file(filename):
    board, agent_pos = read_file(filename)
    return solve_bfs(board, agent_pos)


def solve_a_star_from_file(filename, heuristic):
    board, agent_pos = read_file(filename)
    return solve_a_star(board, agent_pos, heuristic)
...
def test(filename, heuristics):
    ...
    a = time(); solve_bfs(board, agent_pos); bfs_time = time() - a
    times['bfs'] = bfs_time

    for heuristic in heuristics:
        a = time(); solve_a_star(board, agent_pos, heuristic); heuristic_time = time() - a
...
def test_default():
    test_many(['2.' + str(i) + '.in' for i in [2, 3, 4, 7, 8, 9, 10]],
