...
def add_crate(board, pos):
    y, x = pos
    if board[y][x] in {empty, agent}:
        board[y][x] = crate
    elif board[y][x] in {goal, agent_on_goal}:
        board[y][x] = crate_on_goal


# def remove_agent(board, pos):
#     y, x = pos
#     if board[y][x] == agent:
#         board[y][x] = empty
#     elif board[y][x] == agent_on_goal:
#         board[y][x] = goal


# def add_agent(board, pos):
#     y, x = pos
#     if board[y][x] in {empty, crate}:
#         board[y][x] = agent
#     elif board[y][x] in {crate, crate_on_goal}:
#         board[y][x] = agent_on_goal


def board_after_crate_move(board, crate, new_crate):
    new_board = deepcopy(board)
    remove_crate(new_board, crate)
    add_crate(new_board, new_crate)
    return new_board
...
def test_default3():
    test_many(['3.' + str(i) + '.in' for i in [3, 4, 5, 7, 8, 10]],
              [trivial_heuristic])
...
# sufficient heuristic for each case:
# 1 - count_crates_not_on_goals
# 2 - count_crates_not_on_goals
# 3 - greedy_manhattan_distance
# 4 - greedy_manhattan_distance BAD
# 5 - greedy_manhattan_distance BAD
# 6 - count_crates_not_on_goals
# 7 - greedy_manhattan_distance BAD
# 8 - count_crates_not_on_goals BAD
# 9 - count_crates_not_on_goals
# 10 - count_crates_not_on_goals