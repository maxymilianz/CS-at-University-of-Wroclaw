...
def komandos_validator(case, process_out, message=""):
    ...
    if solved_fraction == 1:
        moves_count = len(k_moves)
        if moves_count > max_num_moves:
            fail(message + "Level solved, but path is too long! (" + str(moves_count) + ", should be " + str(max_num_moves) + ")")
        else:
            if VERBOSE:
                print("Level solved!")
            return {'num_moves': moves_count}
    else:
