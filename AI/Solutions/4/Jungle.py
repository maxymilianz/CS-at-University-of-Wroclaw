...
def get_pieces_board():
    # result = [[empty] * w] * h
    #
    # for piece, (y, x) in get_pieces_0().items() | get_pieces_1().items():
    #     result[y][x] = piece
    #
    # return result
...
class Player:
    def __init__(self, den, pieces):
        self.den = den
        self.pieces = self.rat, self.cat, self.dog, self.wolf, self.leopard, self.tiger, self.lion, self.elephant = pieces
    ...
    def tiger_lion_moves(self, piece, y_pos, x_pos, pieces_board):
        if (y_pos, x_pos) not in self.tiger_lion_in_water:
            ...
            return {(y_pos, x_pos)} if pieces_board[y_pos][x_pos] == empty or (
...
class Agent(Player):
    def __init__(self, den, pieces, games_per_move):
        super().__init__(den, pieces)
        self.games_per_move = games_per_move
    ...
    def play(self, pieces, pieces_board):
        ...
        for piece, pos in possibilities:
            quality[(piece, pos)] = 0

            for i in range(ceil(self.games_per_move / len(possibilities))):
                player0, player1 = RandomPlayer(den_0, get_pieces_0()), RandomPlayer(den_1, get_pieces_1())
                if self.den == den_1:
                    player0, player1 = player1, player0
                result = play(RandomPlayer(self.den, get_pieces_0()), RandomPlayer(den_1, get_pieces_1()), deepcopy(pieces_board))
                if result == (0 if self.den == den_0 else 1):
                    quality[(piece, pos)] += 1

        return max(quality, key = lambda x: quality[x])
...
class HeuristicAgent(Player):       # 10/10 wins with Agent and RandomPlayer
...
def determine_winner(player0, player1, pieces0, pieces1, pieces_board):
    pieces_xor = pieces0.keys() ^ pieces1.keys()
    if pieces_xor:
        return 0 if max(pieces_xor, key = lambda piece: power[piece]) in pieces0 else 1

    for dist0, dist1 in zip(sorted(manhattan_distance(*pos, *player1.den) for pos in pieces0.values()),
                            sorted(manhattan_distance(*pos, player0.den) for pos in pieces1.values())):
        if dist0 < dist1:
            return 0
        elif dist0 > dist1:
            return 1
...
def play(player0, player1, pieces_board, debug = False):
    ...
    while True:
        ...
        if not piece:
            ...
            if turns_without_capture >= 50:
                if debug:
                    print('50 turns without capture')
                return determine_winner(player0, player1, pieces0, pieces1, pieces_board)
        ...
        if board[y][x] == den:
            if debug:
                print('Den achieved')
            return 0 if player0turn else 1
...
def play_default(player0, player1):
    return play(player0, player1, get_pieces_board(), False)


def play_agent_vs_random(games_per_move):
    return play_default(Agent(den_0, get_pieces_0(), games_per_move), RandomPlayer(den_1, get_pieces_1()))


def play_heuristic_vs_random(heuristic):
    return play_default(HeuristicAgent(den_0, get_pieces_0(), heuristic), RandomPlayer(den_1, get_pieces_1()))


def play_heuristic_vs_agent(heuristic, games_per_move):
    return play_default(HeuristicAgent(den_0, get_pieces_0(), heuristic), Agent(den_1, get_pieces_1(), games_per_move))


def test_n(n):      # TODO refactor
    agent_wins = 0

    for i in range(n):
        agent_wins += play_heuristic_vs_agent(den_manhattan_distance, 4000)
        print(agent_wins, i)
