...
template<typename N> polynomial<N> operator *(const polynomial<N> &pol1, const polynomial<N> &pol2) {
    std::map<powerproduct, N, powerproduct::cmp> temp;

    for (auto i = pol1.begin(); i != pol1.end(); i++)
        for (auto j = pol2.begin(); j != pol2.end(); j++)
			temp[i->first * j->first] += i->second * j-> second;
