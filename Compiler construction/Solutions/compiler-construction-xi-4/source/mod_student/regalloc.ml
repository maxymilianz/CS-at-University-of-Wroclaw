...
module Make(Toolbox:Iface.COMPILER_TOOLBOX) = struct
  ...
  module Implementation(M:sig
    ...
    let simplify spill_costs interference_graph =
      let rec aux stack spill_costs interference_graph =
        let open ControlFlowGraph in
        let vertexes = List.(sort (fun a b -> compare_lengths
          (successors interference_graph a)
          (successors interference_graph b))
          (labels interference_graph)) in
        match vertexes with
        | [] -> stack
        | vertex :: vertexes ->
          match vertex with
    ...
    let single_pass () =
      ...
      let stack = simplify spill_costs infg in
