...
int checkedForNeighbourBombs[size][size] = { 0 };
...
void checkNeighbourBombs(int x, int y) {		// IT MAYBE COULD BE RECURSIVE
	int neighbourBombs = 0;

	for (int i = x - 1; i < x + 2; i++) {
		for (int j = y - 1; j < y + 2; j++) {
			if (i >= 0 && i <= 8 && j >= 0 && j <= 8 && checkedForNeighbourBombs[i][j] == 0) {
				if (bombBoard[i][j] == 1)
					neighbourBombs++;

				checkedForNeighbourBombs[i][j] = 1;
