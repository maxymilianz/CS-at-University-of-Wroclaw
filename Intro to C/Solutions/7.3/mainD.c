...
{
    if(x1>=0 && x1<x &&  y1>=0 && y1<y && tab[y1][x1]!='#')
        return 1;
    else
        return 0;
}
...
void check (int y, int x,int i,int p1,char**tab)
{
    ...
    switch (c) {
    case 'G':
        if(if_empty(y-1,x,tab) && tab1[y][x]!='G') {
            tab1[y][x]='G';
            check(y-1,x,i+1,p1+1,tab);
            tab1[y][x]='0';
        }

        break;
    case 'D':
        if(if_empty(y+1,x,tab)&& tab2[y][x]!='D') {
            tab2[y][x]='D';
            check(y+1,x,i+1,p1+1,tab);
            tab2[y][x]='0';
        }
        ...
        break;
    case 'P':
        if(if_empty(y,x+1,tab) && tab3[y][x]!='P') {
            tab3[y][x]='P';
            check(y,x+1,i+1,p1+1,tab);
            tab3[y][x]='0';
        }

        break;
    case 'L':
        if(if_empty(y,x-1,tab)&& tab4[y][x]!='L') {
            tab4[y][x]='L';
            check(y,x-1,i+1,p1+1,tab);
            tab4[y][x]='0';
        }
