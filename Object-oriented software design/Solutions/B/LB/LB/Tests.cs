...
namespace LB {
	...
	class Tests {
		...
		public void PropertyInjectionTest() {
			SimpleContainer container = new SimpleContainer();
			container.RegisterType<Foo>(false);
			container.RegisterType<Class0>(false);
			
			Assert.NotNull(container.Resolve<Class0>().FooObject);
		}

		[Test]
		public void NoSetterPropertyInjectionTest() {
			SimpleContainer container = new SimpleContainer();
			container.RegisterType<Foo>(false);
			container.RegisterType<Class1>(false);
			
			Assert.Null(container.Resolve<Class1>().FooObject);
		}

		[Test]
		public void PrivateSetterPropertyInjectionTest() {
			SimpleContainer container = new SimpleContainer();
			container.RegisterType<Foo>(false);
			container.RegisterType<Class2>(false);
			
			Assert.Null(container.Resolve<Class2>().FooObject);
		}

		[Test]
		public void CyclePropertyInjectionTest() {
			SimpleContainer container = new SimpleContainer();
			container.RegisterType<Class3>(false);
			container.RegisterType<Class4>(false);
			
			Exception exception = Assert.Throws<Exception>(() => container.Resolve<Class3>());
			Assert.That(exception.Message, Is.EqualTo(SimpleContainer.ResolveCycleException));
		}

		[Test]
		public void NoAttributePropertyInjectionTest() {
			SimpleContainer container = new SimpleContainer();
			container.RegisterType<Foo>(false);
			container.RegisterType<Class5>(false);

			Class5 instance = container.Resolve<Class5>();

			Assert.Null(instance.FooObject);
		}
		...
		public void BuildUpTest() {
			...
			Assert.NotNull(instance.FooObject);
		}

		[Test]
		public void ServiceLocatorSingletonTest() {
			ServiceLocator locator0 = ServiceLocator.Current;
			ServiceLocator locator1 = ServiceLocator.Current;
			
			Assert.AreSame(locator0, locator1);
		}

		[Test]
		public void ServiceLocatorSingletonDelegateTest() {		// also tests for getting instance of SimpleContainer
			SimpleContainer container = new SimpleContainer();
			ServiceLocator.SetContainerProvider(() => container);
			ServiceLocator locator = ServiceLocator.Current;
			
			SimpleContainer container1 = locator.GetInstance<SimpleContainer>();
			SimpleContainer container2 = locator.GetInstance<SimpleContainer>();

			Assert.AreSame(container1, container2);
		}

		[Test]
		public void ServiceLocatorDelegateTest() {
		...
		public void ServiceLocatorGetInstanceTest() {
			...
			Foo foo = locator.GetInstance<Foo>();
	...
	public class Class3 {
		[DependencyProperty]
		public Class4 Object { get; set; }
	}

	public class Class4 {
		[DependencyProperty]
		public Class3 Object { get; set; }
	}
